...
class Main:
    ...
    @staticmethod
    def main():
        process = sys_io_Process("adb shell pm list package")
        _hx_list = process.stdout.readAll().toString()
        if (process.exitCode() != 0):
            raise haxe_Exception.thrown("未链接设备、设备未授权或者设备不支持adb shell命令")
        clearPkg = python_internal_ArrayImpl._get(Sys.args(), 0)
        if (clearPkg is None):
            raise haxe_Exception.thrown("请提供至少两个节点的包名：*.*")
        print(str((("开始清理对应包名的安装包：" + ("null" if clearPkg is None else clearPkg)) + ".*")))
        apks = _hx_list.split("\n")
        _g = 0
        while (_g < len(apks)):
            item = (apks[_g] if _g >= 0 and _g < len(apks) else None)
            _g = (_g + 1)
            startIndex = None
            if (((item.find(clearPkg) if ((startIndex is None)) else HxString.indexOfImpl(item,clearPkg,startIndex))) != -1):
                startIndex1 = None
                item = HxString.substr(item,(((item.find(":") if ((startIndex1 is None)) else HxString.indexOfImpl(item,":",startIndex1))) + 1),None)
                code = Sys.command(("adb uninstall " + ("null" if item is None else item)))
                print(str((("清理：" + ("null" if item is None else item)) + HxOverrides.stringOrNull(((("[" + HxOverrides.stringOrNull((("成功" if ((code == 0)) else "失败")))) + "]"))))))
...
class Sys:
    _hx_class_name = "Sys"
    __slots__ = ()
    _hx_statics = ["args", "systemName", "command"]
